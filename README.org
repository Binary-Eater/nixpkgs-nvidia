...
#+BEGIN_SRC nix
  ...
  let
    ...
    nvidiaPkgs = import (builtins.fetchTarball https://github.com/Binary-Eater/nixpkgs-nvidia/archive/trunk.tar.gz) {
      inherit lib stdenv pkgsi686Linux;
  ...
  in
  {
    ...
    hardware.nvidia = {
      package = nvidiaPkgs.latest; # see "Available packages" below for other options
...
The package set tracks the [[https://www.nvidia.com/en-us/drivers/unix/][NVIDIA Unix Drivers]] page. Each branch is
exposed as a top-level attribute on the imported set:

- ~production~ :: Latest Production Branch release.
- ~new_feature~ :: Latest New Feature Branch release.
- ~beta~ :: Latest Beta release.
- ~latest~ :: Highest stable version between ~production~ and ~new_feature~.
  Recommended default.
- ~bleeding_edge~ :: Highest version between ~latest~ and ~beta~. Pick this
  if you want the absolute newest release regardless of stability label.
- ~mkDriver~ :: Lower-level function for building an arbitrary version. See
  ~generic.nix~ for the accepted arguments.
